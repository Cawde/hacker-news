...
const Search = ({history, setHistory, keywords}) => {
  ...
  const getSearchResults = () => {
    ...
    fetch(`http://hn.algolia.com/api/v1/search?query=${searchTerm}&page=${pageNumber - 1}`)
    ...
    <div className="search-page">
      ...
      <div className="search-box">
        ...
        <div className="pagination">
          {pageNumber > 0 ? <button onClick={(e) => {
            e.preventDefault();
            setPageNumber(pageNumber - 1);
          }}>Previous</button> : null}

          {pageNumber < 50 ? <button onClick={(e) => {
            e.preventDefault();
            setPageNumber(pageNumber + 1);
          }}>Next</button> : null}
        </div>
        ...
        {results ? results.map((result, index) => {
          return (
            <div className="search-results" key={index}>
              ...
              <a href={result.url} target="_blank" rel="noreferrer" onClick={() => {setHistory([...history, {term: searchTerm, url: result.url}])}}>{result.url}</a>
